...
@section('content')
    @if (auth()->user()->can('Editar Leitura das Concessionárias') &&
        ...
        @php
            ...
            foreach ($readings as $reading) {
                $list[] = [$reading->id, $reading->complex['alias_name'], $reading->month_ref, $reading->year_ref, $reading->reading_date, $reading->reading_date_next, '<nobr>' . '<a class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar" href="dealerships-readings/' . $reading->id . '/edit"><i class="fa fa-lg fa-fw fa-pen"></i></a>' . '<a class="btn btn-xs btn-default text-danger mx-1 shadow" title="Excluir" href="dealerships-readings/destroy/' . $reading->id . '" onclick="return confirm(\'Confirma a exclusão deste consumo?\')"><i class="fa fa-lg fa-fw fa-trash"></i></a>'];
            }
    ...
    @else
        @php
            $heads = [['label' => 'ID', 'width' => 5], 'Condomínio', 'Mês Ref', 'Ano Ref', 'Data da Leitura', 'Próx Leitura'];

            $list = [];

            foreach ($readings as $reading) {
                $list[] = [$reading->id, $reading->complex['alias_name'], $reading->month_ref, $reading->year_ref, $reading->reading_date, $reading->reading_date_next];
            }
