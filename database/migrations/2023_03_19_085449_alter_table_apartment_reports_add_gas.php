...
class AlterTableApartmentReportsAddGas extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::table('apartment_reports', function (Blueprint $table) {
            $table->decimal('consumption_gas_value', 13, 3)->nullable();
            $table->decimal('total_gas_value', 13, 3)->nullable();
        });

        DB::statement("
        CREATE OR REPLACE VIEW apartment_reports_view AS
        SELECT ar.id, CONCAT(c.alias_name, ', Bl. ', bl.name , ', Ap. ', ap.name  ) as unit,
        c.alias_name as complex, bl.name as block, ap.name as apartment, ar.consumed,
        ar.consumed_cost, ar.sewage_cost, ar.kite_car_consumed, ar.kite_car_cost, ar.partial,
        ar.total_unit, ar.dealership_reading_id, ar.apartment_id, dr.month_ref, dr.year_ref, ar.consumption_gas_value, ar.total_gas_value
    ...
    {
        Schema::table('apartment_reports', function (Blueprint $table) {
            $table->dropColumn(['consumption_gas_value', 'total_gas_value']);
        });
